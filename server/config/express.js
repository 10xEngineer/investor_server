...
module.exports = function(app, passport, db) {
  ...
  //HACK Only use logger for development environment
...
/*  app.use(function(req, res, next) {
    if(!req.secure) {
	  var http_url = 	['https://',
							( req.get('Host').match(/:/g) ) ? req.get('Host').slice( 0, req.get('Host').indexOf(":") ) : req.get('Host'),
							req.url].join('');
	  console.log('redirect to: '+http_url);
